...
def solution(field):
    ...
    - 최아그이 while True 루프 횟수  = h x w / 4(한번에 4개만 터짐)
    핵심 아이디어: dfs, routes dict 로 그룹을 기록
    - 풀고 나서 보니 bfs 로 풀었으면 그룹대로 방문하므로 routes 딕셔너리가 따로 필요 없음
    """  
    h, w = 12, 6
    ...
    while True:
        ...
        prev_answer = answer
        bomb = 0
        for route_list in routes.values(): # O(hxw)
            if len(route_list) >= 4:
